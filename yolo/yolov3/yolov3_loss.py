...
def build_targets_New(target, anchors, num_anchors, inh, inw, nclass, sil_thresh):
    target = target.data
...
class Yolov3Loss_New(nn.Module):
    def __init__(self, num_classes=2, anchors=[], num_anchors=3, stride=32):
        super(Yolov3Loss_New, self).__init__()
        self.num_classes = num_classes # 20,80
        self.num_anchors = num_anchors #3
    ...
    def forward(self, output, target):
        ...
        anchor_w = scaled_anchors[:, 0].repeat(nB, 1).repeat(1, 1, nH * nW).view(nB, nA, nH, nW)
        anchor_h = scaled_anchors[:, 1].repeat(nB, 1).repeat(1, 1, nH * nW).view(nB, nA, nH, nW)
        ...
        obj_mask, noobj_mask, tx, ty, tw, th, tconf, tcls = build_targets_New(target, scaled_anchors, nA, nH, nW, nC, self.thresh)
